---
title: 'Dickson Lab: Lung Microbiome 16S Analysis Tutorial'
author: "Christopher A. Brown"
date: "July 31, 2021 (updated)"
output:
  html_document:
    code_folding: show
    df_print: paged
...
# I. Introduction
Welcome to the Dickson Lab's introduction to studying the lung microbiome. In this tutorial we'll see how we can use R to perform data manipulation, modeling, visualization, and reporting as part of our analysis workflow. If you have not already done so, please download R, RStudio, and the necessary packages. See this [guide](https://github.com/cb-42/Dickson_16S_SOP) for more details.
...
paste("Welcome to the Dickson Lab SOP for Analysis of 16S rRNA Gene Amplicon Data")
```
...
# The readxl library is installed along with tidyverse, but is not loaded with library(tidyverse). We can however access the read_xlsx function without loading readxl as shown below with the readxl::read_xlsx notation.
...
In this subsection we will explore visualization methods including PCA ordinations, biplots, rank abundance plots (ordered bar charts), and heat maps.  
...
# This is very similar to the previous ordination except that we're drawing ellipses rather than lines connecting the points to the centroids.
plot(otu_con_lung_pca, type = "n", font = 2, font.lab = 2, xlab = "PC1 (17.82% Explained)", ylab = "PC2 (8.60% Explained)", main = "PCA of Lung Tissue vs Controls, colored by Type", display = "sites")
points(otu_con_lung_pca, pch = 19, col = pcapal[as.numeric(con_lung_df$reagent_tissue)]) # pch determines the point shape and col determines the colors to be used.
ordispider(otu_con_lung_pca, con_lung_df$reagent_tissue, label = TRUE) # Draw lines from centroids to the points belonging to each group.
...
points(otu_con_lung_pca, pch = 19, col = pcapal[c(5,2)][as.numeric(con_lung_df$reagent_tissue)]) # color palette adjusted to conflict less with biplot arrows
...
otu_lung <- dplyr::filter(otu_df, tissue_type == "Lung") %>%
  droplevels() # remove levels corresponding to data that is no longer present due to filtering
...
Let's also scale the point size with a quantitative variable and change point opacity. Note that it is easier to judge absolute differences in a quantitative variable using bars (for example) rather than point areas, but this can give us a general idea about the the measurements.  

```{r spider_lung_scaled, fig.height=10, fig.width=10}
# Now let's scale the point size by a quantitative measure (log_IL1a) and adjust the point opacity
plot(otu_lung_pca, type = "n", font = 2, font.lab = 2, xlab = "PC1 (14.58% Explained)", ylab = "PC2 (10.68% Explained)", main = "PCA of Specimen Origin, colored by Vendor", display = "sites")
points(otu_lung_pca, pch = 19, col = alpha(pcapal[as.numeric(otu_lung$vendor)], 0.6), cex = 2*otu_lung$log_IL1a) # Note the use of alpha around the color vector and cex for the size. The size scale is doubled to make differences more apparent.
ordispider(otu_lung_pca, otu_lung$vendor, label = TRUE) # Draw lines from centroids to the points belonging to each group.
legend("topright", levels(otu_lung$vendor), pch = 19, col = pcapal, title = "Vendor") # Format and add details to legends
```

Finally, suppose we want to examine PC1 and PC3 instead of PC1 and PC2. The `choices` parameter allows us to select PCs besides the PC1 and PC2 defaults. We'll also need to update the axis labels accordingly.

```{r spider_lung_scaled_pc1_pc3, fig.height=10, fig.width=10}
# This approach combines base R plotting functionality and vegan's ordispider function, which creates the lines connecting points to the centroids.
# Unlike with ggplot(), these lines must all be run at once in order to produce the desired result.
plot(otu_lung_pca, choices = c(1,3), type = "n", font = 2, font.lab = 2, xlab = "PC1 (14.58% Explained)", ylab = "PC3 (6.78% Explained)", main = "PCA of Specimen Origin, colored by Vendor", display = "sites")
points(otu_lung_pca, choices = c(1,3), pch = 19, col = alpha(pcapal[as.numeric(otu_lung$vendor)], 0.6), cex = 2*otu_lung$log_IL1a) # Note the use of alpha around the color vector and cex for the size. The size scale is doubled to make differences more apparent.
ordispider(otu_lung_pca, choices = c(1,3), otu_lung$vendor, label = TRUE) # Draw lines from centroids to the points belonging to each group.
legend("topright", levels(otu_lung$vendor), pch = 19, col = pcapal, title = "Vendor") # Format and add details to legends
```
...
#### 4.1.2.b Base implementation & dendrogram  
...
#### 4.1.3.a Comparison of Negative Controls against Lung Tisssue specimens  
...
ra_df <- otu_df %>% # Note that this is just one of several possible approaches to filtering to retain the correct rows
  dplyr::filter((reagent_tissue == "Control") | (tissue_type == "Lung")) %>% # 65 specimens
...
tax_df$OTU <- factor(tax_df$OTU, levels = otu_order[1:20]) # set OTU factor levels to be in the same order as in g_df. If this is not done, our OTU column will be coerced to a character vector when we join (we'll lose the factor levels)
...
(q <- q + theme_bw() + # note that a theme layer added after other other theme modifications will remove these prior modifications
...
# Try to experiment with the above code by running it again after commenting out a layer, or altering 1 parameter. This will help you to develop a deeper understanding of how to build up or alter a ggplot object.
```

#### 4.1.3.b Comparison of Lung Tisssue specimens by vendor  
...
fam_df <- otu_df %>%
  ...
  dplyr::select(specimen, reagent_tissue:IL1a, everything()) # Pull metadata columns to the front, all Family columns follow afterwards
```
...
## 7.3 Saving (or loading) a model 
The following code chunk illustrates how a random forest model can be saved and read in. Note that `saveRDS` potentially allows you to save any R object.

```{r saveRDS}
# Here's how you can save and/or load a model
saveRDS(rf_vendors, "rf_vend_model.rds")
rf_vendors2 <- readRDS("rf_vend_model.rds") # Saved to a new object to allow comparison (it should contain identical information)
```

## 7.4 Generating a report: Knitting  
...
## 8.4 Dickson Lab 16S SOP, ATS 2021 PG11 course materials, cbmbtools  
[https://github.com/cb-42/Dickson_16S_SOP](https://github.com/cb-42/Dickson_16S_SOP)  
    - This repository houses the code and data used for the Dickson Lab 16S Analysis SOP.  
