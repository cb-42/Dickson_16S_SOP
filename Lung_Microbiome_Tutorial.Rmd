---
title: 'Dickson Lab: Lung Microbiome Analysis Tutorial'
author: "Christopher A. Brown"
date: "September 29, 2020 (updated)"
output:
  html_document:
    code_folding: show
    df_print: paged
...
avg_abs_df <- abs_df %>%
  ...
  summarize(mean_DNA_copies = mean(DNA_copies_per_ul), SEM = sqrt(var(DNA_copies_per_ul)/length(DNA_copies_per_ul))) %>% # Create mean and SEM columns
  ungroup()
```
...
# example for replacing specimen labels with C or T
...
fam_df <- otu_df %>%
  ...
  dplyr::summarize(Percentage = sum(Percentage)) %>% # Note that this step removes any metadata columns which aren't grouped by
  dplyr::ungroup() %>%
...
In this section we'll explore measures of community diversity and how they can be used within our analyses.    

## 6.1 α-diversity: within specimens
First, we'll look at the diversity within specimen  

### Richness - how many unique taxa?  

### Evenness - how evenly distributed?  

We'll compute the Shannon diversity with the `diversity()` function. Then we'll select the observations of interest and compute summary statistics.

```{r alpha_div}
shan_div <- diversity(otu_good) %>% 
  data.frame(Shannon = .) %>%
...
# Create summary statistics for bars/error bars  
agg_shan_lung <- shan_lung %>%
  group_by(vendor) %>%
  summarize(Mean_Sh = mean(Shannon), SEM_Sh = sqrt(var(Shannon)/length(Shannon))) # Create summary statistics
```

To visualize the results, we'll plot the specimens' diversities and means using `ggplot()`.
...
# TukeyHSD(aov(shan_lung$Shannon ~ shan_lung$vendor)) # This code produces the same result as above
```
...
rarefy_agg <- rarefy(dplyr::select(otu_raw, -label, -numOtus), sample = 1:1000) %>% # subsample for each number in 1:1000; an alternative is to set sample = to the integer length of primary interest, e.g. 1000
  ...
  summarize(Mean_Species = mean(Unique_Species), SEM = sqrt(var(Unique_Species)/length(Unique_Species))) # Create summary statistics by vendor (negative controls are considered 'None')
...
### Bray-Curtis  
As wikipedia notes: [Bray–Curtis dissimilarity](https://en.wikipedia.org/wiki/Bray%E2%80%93Curtis_dissimilarity) is a is a statistic used to quantify the compositional dissimilarity between two different sites, based on counts at each site.   

### Unifrac  
Future example. [Descriptive text here]  

# VII. Reporting  
Usually it's not enough that you've generated statistics or created visualizations. You also need to be able to share this work. Fortunately, there are a variety of ways to do this.
...
write.csv(otu_lung, file = "otu_lung.csv")
```

## 7.2 Saving a plot  
Next, we'll save a plot. Note that we will also have access to a folder with all of the plots after finishing step 7.3.  
